...
::: warning ⚠️ 模型名稱的時效性提醒
本章節中提及的具體模型名稱（如 `gemini-2.5-pro`、`claude-4-sonnet`）是截至本教程撰寫時的範例。AI模型技術日新月異，未來可能出現更新、更強大的模型。

學習本章的關鍵在於掌握**「思考型」**與**「執行型」**的核心分類原則，而非記憶特定模型名稱。請將這些原則靈活應用於您當前可用的最新模型上。
:::
